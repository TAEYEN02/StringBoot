...
class AppContext {
	...
	private void doComponentScan() {
		try {
			...
			// ClassLoader
			// JVM내부에서 클래스와 리소스(설정 파일, 이미지 등)을 로딩하는 역할을 하는 객체

			// AppContext.class.getClassLoader()
			// AppContext클래스를 로딩한 ClassLoader객체를 반환
			ClassLoader classLoader = AppContext.class.getClassLoader();

			// ClassPath는 구아바(Guava) 라이브러리에서 제공하는 기능으로,
			...
			for (ClassPath.ClassInfo classInfo : set) {
				...
				// 아래의 로직을 실행해라
				if (component != null) {
					// 클래스의 이름의 첫글자를 소문자로 변환하여 id로 사용할것이다.
					// 변환하는 이유는 스프링에서 빈을 생성할 때, 기본적으로 클래스 이름의 첫글자를
					// 소문자로 사용하기 때문이다.
					// getSimpleName() : 패키지 없이 클래스 이름만 변환
...
public class Main {
	public static void main(String[] args) {
		...
		//실제로는 @ComponentScan어노테이션으로 모든과정을 퉁친더
